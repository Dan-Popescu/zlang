...
int interpret(Interpreter *interpreter, ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case ASSIGNMENT_NODE: {
            ...
            const char *var_name = node->node->assignOpNode->identifier->node->variableNode->varToken->value.strValue;
            const VariableScope *var_scope_found = find_variable_in_global_scope(interpreter->global_scope, var_name);
            const int value = var_scope_found->value.intValue;
            return value;
        }
...
int visit_bin_op_node(Interpreter *interpreter, const ASTNode *node) {
    ...
    const int left_value = visit_node(interpreter, node->node->binaryOpNode->left);
    const int right_value = visit_node(interpreter, node->node->binaryOpNode->right);
...
int visit_number_node(Interpreter *interpreter, const ASTNode *node) {
    const NumberNode *numNode = node->node->numNode;
    //    printf("%d ", numNode->value);
    return numNode->value;
}

int visit_unary_op_node(Interpreter *interpreter, const ASTNode *node) {
    const int expr_value = visit_node(interpreter, node->node->unaryOpNode->expression);
...
void visit_assign_node(Interpreter *interpreter, const ASTNode *node) {
    const char *var_name = node->node->assignOpNode->identifier->node->variableNode->varToken->value.strValue;
    const int value = visit_node(interpreter, node->node->assignOpNode->expression);
...
GLOBAL_SCOPE *init_global_scope(unsigned short initialCapacity) {
    ...
    if (globalScope->variables == NULL) {
        ...
        free(globalScope); // free memory allocated for global scope
        exit(EXIT_FAILURE);
    }
...
VariableScope *find_variable_in_global_scope(GLOBAL_SCOPE *globalScope, const char *varName) {
    for (unsigned short idx = 0; idx < globalScope->size; ++idx) {
        if (globalScope->variables[idx] != NULL && globalScope->variables[idx]->variableNode != NULL &&
            ...
            const char *gl_scope_var_name = globalScope->variables[idx]->variableNode->varToken->value.strValue;
...
void set_variable_in_global_scope(GLOBAL_SCOPE *global_scope, VariableScope *var_scope) {
    ...
    const char *var_name_searched = var_scope->variableNode->varToken->value.strValue;
...
void add_variable_to_global_scope(GLOBAL_SCOPE *globalScope,
    ...
    const unsigned short size = globalScope->size;
